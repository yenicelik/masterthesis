...
def static_principal_component(matr):
    ...
    print("Input matrix has shape: ", matr.shape)
    pca_model = PCA()
    pca_model.fit(matr)
    principal_component = pca_model.components_[0, :]
    print("Principal component shape")
    print(principal_component.shape)
    principal_variance = pca_model.explained_variance_[0]

    print("Principal variance shape")
    print(principal_variance.shape)
...
def sample_principal_components_from_cluster_centers(X, cluster_labels, n=10):
    ...
    for label in np.unique(cluster_labels):
        ...
        print("idx is: ", idx)
        print("idx is (2): ", idx.shape)
        # Create a principal component from this ...
        print("Into the wild: ", X.shape)
        print("Into the wild (2): ", X[idx, :].shape)
...
def return_gaussian_mixture_from_principal_components(cluster_center_variance_pairs):
    ...
    sess = tf.Session() # TODO: Move these to a new session outside (else the variables may be deleted)
...
if __name__ == "__main__":
    ...
    rnd_str = randomString(
        additonal_label=f"_wordembeddings_{args.dimred}_{args.dimred_dimensions}_whiten{args.pca_whiten}_norm{args.normalization_norm}")
    ...
    for word in ['state']:  # get_words_in_benchmarks("simlex"):
        ...
        print("Loaded matr has shape: ", matr.shape)

        pvec, pvar = static_principal_component(matr)

        print("Principal vector and principal variable are: ")
        print(pvec.shape, pvar)

        # Question is do we want to preprocess these
        # X_hat = matr  # preprocess_matrix(matr)
        ...
        #
        # centers_and_variances = sample_principal_components_from_cluster_centers(
        #     X=X_hat,
        #     cluster_labels=clusters_hat
        # )
        #
        # print("Shapes for centers and variances are")
        # print([(x[0].shape, x[1].shape) for x in centers_and_variances])

        # gmm_pdf = return_gaussian_mixture_from_principal_components(centers_and_variances)

        # gp_pdf = return_gaussian_process_from_datasamples(matr)
