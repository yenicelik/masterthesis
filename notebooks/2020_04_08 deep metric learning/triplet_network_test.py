"""
    Implement a triplet network, because this implicitly implements a margin-loss!
"""
import random
import numpy as np
...
class Siamese(nn.Module):
    ...
    def forward(self, input1, input2):
        ...
        # Remove the sqrt!
        distance = torch.sqrt(
            torch.sum((output1 - output2) * (output1 - output2), 1) + 0.1
        )
...
def siamese_train(model, X1s, X2s, ys, optimizer):
    """
        X_tpl is a tuple.
    ...
    """
    model.train()
    assert X1s.shape[0] == X2s.shape[0]
    assert X2s.shape[0] == ys.shape[0]
