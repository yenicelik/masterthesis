...
This makes a formal analysis of our topic easier.
...
We now also employ multi-class classification.
...
In the case of BERNIE Cluster, we use the performance of the downstream task as a proxy to understand what information is captured by the high variance.
...
As such, the initial idea is to introduce additional embedding vectors which are able to capture semantics (rather than just pure word tokens) better.
...
Similar to the above model, we are introducing new BERT tokens and their respective embeddings in the transformer embedding layer, similar to the previous section \ref{bernie_pos}.
...
We now also modify the tokenizer in such a way, that ambiguous and polysemous words are replaced with a more specific token.
...
For easier explainability, we assume that the different clusters refer to semantic clusters, but this is not always the case as we have seen before.
...
We assume that this is necessary, as this is also how BERT is trained in the original paper \cite{devlin18}.
...
We then run one full epoch of training with the same training parameters as used for GLUE on the news.corpus.2007 dataset using a masked language model methodology as described in section \ref{section:Background}.
We save this model and load it for the GLUE tasks.
BERNIE full Pre-training trains all the embeddings, whereas BERNIE partial Pre-Training fixates all embedding vectors except the ones for the ones that were newly added.
...
As this work only concludes a Master Thesis, we were only able to investigate some of our core hypothesis. 
