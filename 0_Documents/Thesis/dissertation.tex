...
We adopt the terminology used in background and related work, and thus also refer to BERT as a language model, even though it cannot capture arbitrary probability distributions such as $p(w_t|w_{t-1}, \ldots, w_{1})$.
...
\end{verbatim}
\end{tcolorbox}
%TODO The x-axis shows the WordNet sense index for a chosen word. The y-axis shows the log-frequency within SemCor. 
\begin{figure}[h]
	\center
...
\SetKwInOut{Input}{Input}
\Input{
$w_{\text{target}}$: The target word whose vectors we want to cluster; \\ 
...
The scoring scheme we use is the adjusted random index \textit{ARI} \cite{rand71, hubert85} which compares to what extent two clusterings are identical.
...
\captionof{figure}{Partition 5 for the word \texttt{arms}. This cluster again refers to the arms of a person, however usually with a positive / optimistic sentiment.}
\end{tcolorbox}
...
\captionof{figure}{Partition 6 for the word \texttt{arms}. This cluster contains \texttt{arms} in the context of countries (no individuals). This uses the semantic definition of arms which is analogous to \texttt{weaponry}. One can notice that these sentences usually refer to nuclear arms.}
\end{tcolorbox}
...
\begin{figure}
	\center
  \includegraphics[width=0.8\linewidth]{./assets/experiments/pipeline_model_BERnie_meaning.png}
...
\begin{align}
R &= \{ (x_i, x_j, j_l) : x_i \text{ is more similar to } x_j \text{ than  } x_l \} \\
\end{align}
...
Because the coincidence property is not always important to us, but because word-embeddings are more interested in measuring relative distances, we will also consider mappings known as \textit{pseudo-dinstances} which relax the coincidence property to merely fulfill $d(x, x) = 0$.
...
\cite{suarez19} offers a survey on this work.
...
\begin{align}
\texttt{                                              } & \texttt{ - -} \nonumber \\
\texttt{I would expect the} & \texttt{ bank to the left of the road } \nonumber \\
\texttt{                                              } & \texttt{-  -} \nonumber \\
...
\begin{align}
\texttt{                                                    } & \texttt{- -} \nonumber \\
 \texttt{lift mystery surrounding the central} & \texttt{ bank and improve communications with Wall Street} \nonumber \\
\texttt{                                                    } & \texttt{- -} \nonumber \\
...
\texttt{China’s central} & \texttt{ bank has stepped up its  purchases of dollar } \nonumber \\
\texttt{fertilizer prices in Africa, but the} & \texttt{ bank itself had often failed to recognize} \nonumber \\
\texttt{                                                    } & \texttt{- -} \nonumber
\end{align}
\end{minipage}
}
...
\begin{align}
\texttt{                                              } & \texttt{- -} \nonumber \\
\texttt{citing challenges for the investment} & \texttt{ bank and the potential for a credit burden} \nonumber \\
\texttt{                                              } & \texttt{- -} \nonumber \\
...
We now present a full partitioning over the samples of word \texttt{key}. \\
...
\texttt{quickly established a good rapport with} & \texttt{ key donors” } \nonumber \\
...
\begin{align}
\texttt{                                                 } & \texttt{- -} \nonumber \\
\texttt{president of Trinity College, who played a} & \texttt{ key role in designing the test} \nonumber \\
\texttt{                                                 } & \texttt{- -} \nonumber \\
\texttt{seen in the West as a} & \texttt{ key for the fairness of an election} \nonumber \\
\texttt{treat ... as a} & \texttt{ key factor in its regaulatory decision} \nonumber \\
\texttt{which policy makers have called the} & \texttt{ key test to lower interest rates} \nonumber \\
...
\begin{align}
\texttt{                                                 } & \texttt{- -} \nonumber \\
\texttt{Interstate 5 is a} & \texttt{ key route connecting Southern  California} \nonumber \\
\texttt{                                                 } & \texttt{- -} \nonumber \\
\texttt{ A} & \texttt{ key piece of functionality for Ops Center } \nonumber \\
...
\texttt{What would you say was the} & \texttt{ key element of your management style} \nonumber \\
\texttt{is a} & \texttt{ key indicator of retailer performance} \nonumber \\
\texttt{in the West the} & \texttt{ key players were not a small group reading } \nonumber \\
\texttt{                                                 } & \texttt{- -} \nonumber
\end{align}
\end{minipage}
}
...
\texttt{                                                 } & \texttt{- -} \nonumber \\
\texttt{And an official, five-inch-long gold-plated pewter} & \texttt{ key to prove it} \nonumber \\
\texttt{but it is small enough to fit onto a} & \texttt{ key chain} \nonumber \\
\texttt{In it lay three keys on a} & \texttt{ key chain in the shape of } \nonumber \\
\texttt{                                                 } & \texttt{- -} \nonumber
\end{align}
\end{minipage}
}
...
\begin{align}
\texttt{                                                 } & \texttt{- -} \nonumber \\
\texttt{The Red Sox will now have all their} & \texttt{ key players from the 2007 championship team} \nonumber \\
\texttt{                                                 } & \texttt{- -} \nonumber \\
\texttt{Mike Green scored 12 points and had a} & \texttt{ key assist in overtime as Butler beat} \nonumber \\
\texttt{or taking the chance of losing a} & \texttt{ key player to injury} \nonumber \\
\texttt{But they never led, could not get a} & \texttt{ key basket at crucial times and played like} \nonumber \\
  \texttt{Cam Long stole the ball near the top of the} & \texttt{ key and ran out the clock} \nonumber \\
\texttt{                                                 } & \texttt{- -} \nonumber
\end{align}
\end{minipage}
}
\captionof{figure}{Partition 5 for the word \texttt{key}. This cluster contains \texttt{key} as a synonym for the word \texttt{main}, which characterizes an important subject. The context is one about physical activity. The sentiment is mixed.}
\end{tcolorbox}
...
\begin{align}
\texttt{                                                 } & \texttt{- -} \nonumber \\
\texttt{Three of their} & \texttt{ key players played more than 40 minutes} \nonumber \\
\texttt{                                                 } & \texttt{- -} \nonumber \\
\texttt{A} & \texttt{ key for the Giants on Sunday} \nonumber \\
\texttt{chemical reactions on solid surfaces, which are} & \texttt{ key to understanding questions like} \nonumber \\
\texttt{but is she part of the conspiracy or the} & \texttt{ key to Sim’s salvation?} \nonumber \\
\texttt{to play on a sprained ankle against the Eagles} & \texttt{ key to that matchup} \nonumber \\
\texttt{Horses have been the lifelong} & \texttt{ key to satisfying real feminine needs} \nonumber \\
\texttt{Connecticut cornerback said the} & \texttt{ key to defeating Louisville} \nonumber \\
\text{                                                 } & \texttt{- -} \nonumber
\end{align}
\end{minipage}
}
...
\label{appendix:tensorboard_visualizations}
