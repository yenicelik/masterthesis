...
%TODO Introduce nominalised verbs
%TODO Replace all "I" by "We"
...
Most commonly, language models create a word embedding by taking into consideration the context with which a word token occurs. 
...
This is not an exhaustive summary, but focuses on the properties of languages relevant to this work. \\
...
For the sentence \ref{sentence:man_travelled}, the set of basic units would correspond to the Latin alphabet plus whitespace and numbers $a - z" "0-9$ (" " denotes whitespace), and the above sentence would be tokenized into the sequence 
...
Here, the most frequent pair of bytes in a sequence is replaced with the most frequent pair of bytes in a sequence with a single, unused byte, similar to a Huffman encoding.
...
The gradients of the ELMo biLSTM model are frozen updated during fine-tuning, only modifying the weights of the task-specific weights. 
...
A softmax layer is added on top of the of dot-products to denote to what extent two vectors correlate to each other.
...
The Bidirectional Encoder Representations from Transformers \textbf{BERT} \cite{devlin18} make use of the transformer architecture in contrast to ELMo which has recurrent neural networks as its underlying architecture.
...
These include GPT \cite{radford18} and GPT-2 \cite{radford19} which are also based on transformers, however only allows the sequence modeling to be forward looking (in contrast to BERT, which also allows the future context to be taken into account when creating the context embedding).
These also include more fundamental modifications to the transformer module, most notably introducing layer normalization \cite{ba16} at various positions in the module. 
...
Because in later experiments, the inference datasets are the most interesting tasks for us. 
...
If the self-similarity is low, this suggests a less nuanced contextualized.
...
Also, it is important to ask oneself what causes the separation between clusters.
Given figure \ref{BERT_plurality}, one cannot reliably say that the part-of-speech feature \textit{causes} the partition into clusters.
...
Figures \ref{fig:example_arms_negative_handcuffed} - \ref{example_arms_negative_crossed} show a a partition that is created when we run the above received best clustering algorithm on the word \Verb#arms#, which is polysemous. 
...
We refer to appendix \ref{more_clustering_results_qualitative} for some more examples.
...
Especially in Figure \ref{fig:run_tensorboard} (a), we can see that in the case of UMAP, the central clsuter strongly mixes nouns with adjectives (red and blue cluster), and that in the case of PCA, the context embeddings are constrained in such a way that embeddings which mix different semantic classes meet in the middle.
...
Our analysis will be limited to BERT only, although similar changes can also be made to modifications of BERT including ALBERT \cite{lan20}, DistillBERT \cite{sanh19}. 
...
Please refer to \ref{section:Background} for a refresher on how sentences are tokenized inside BERT, and how tokens are converted to the latent representation and finally into context embeddings through the BERT model.
\\
...
This happens because we introduce additional parameters that can parametrize a specific token distribution, allowing to approximate the underlying true distribution better.
...
We optimize the hyperparameters in such a way that the number of WordNet classes is closely predicted through the number of clusters found.
The resulting clusters are implicit latent variables that are not hard-coded, and thus cannot be intuitively understood without further analysis.
...
We then run one full epoch of training with the same training parameters as used for GLUE on the news.corpus.2007 dataset using a masked language model methodology.
...
Although this work does not try to imminently push the performance of modern language, we believe that providing a better understanding of the underlying principles will pave the way for future research to better address these issues, and make language models more interpretable especially with respect to semantics. \\

\section{Main Contributions}

%TODO rewrite this sec

Our main contributions are two-fold. 
We first test to what extent the semantic subspace is linearly separable between semantic classes. 
We test for multi-modality in these structures through a clusterability test.
