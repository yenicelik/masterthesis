...
These shortcomings can be addrressed by modelling written text in such a way that linguistic features are captured.
...
The formalization of language is not trivial and may even be considered in a philosphical context \cite{hegel17, wittgenstein53}, posing the question on the nature of the relation between thought and (language)-representation.
...
synonyms are words, where one word is interchangable for another.
...
We now do an evaluation of the introduced BERNIE PoS and BERNIE Cluster models.
...
We assume that this is due to the pre-training corpus not being similar to the pre-training corpus that was initially used for BERT (which is too big to be run for this test), or the hyperparamters not being optimal for this run.
...
The newly introduced embeddings strongly decrease RTE task performance, while the WNLI task performance is strong improved
\end{enumerate}
