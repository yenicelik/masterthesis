...
In contrast  an LSTM would contain a single hidden state $h_{t-1}$ passed forward at every timestep. 
This would have to contain the information for all previous words $h_k$ where $k < t - 1$.
By using multiple attention layers on top each other, BERT is a deep model, allowing for more complex patterns to be trained. \\
...
Although these words are not the most intuitive polysemous words (as would \Verb#bank# be, for example), the limited "obviousness" should allow this experiment to be stricter with respect to not rejecting the hypothesis.
...
We refer to appendix \ref{section:more_clustering_results_qualitative} for some more examples.
...
\captionof{figure}{Partition 2 for the word \texttt{arms}. This cluster contains \texttt{arms} in the context of arms where one person hugs or loves another. Notice that this usually includes a positive sentiment.}\label{fig:example_arms_negative_crossed}
\end{tcolorbox}
...
This happens because we introduce additional parameters that can parametrize a specific token distribution, allowing a better approximation of the underlying true distribution.
...
We use the clustering approach introduced in Section \ref{experiment_BERT_clusterability} as the intermediate model to distinguish on which bank the sentence refers to.
...
\begin{figure}[H]
  ...
  \caption{The BERT model takes as input a sentence $s$. The resulting, fully modifid BERnie Cluster pipeline.}
...
Because we want to understand the effect of these modifications, we focus on an ablation study rather than absolute performance improvement to state-of-the-art methods like ALBERT.
...
However, the WNLI and RTE experiments are considerably deviating in performance through the addition of the additional embedding vectors.
