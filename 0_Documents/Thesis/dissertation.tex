...
The field of Natural Language Processing (\textit{NLP}) aims at developing algorithms that are able to operate with human language.
However, it is not yet apparent that machines can think, or even operate with semantic concepts.
In most of these cases, the underlying language model uses heuristics, or black-box function which simply maps an input text to output text.
 
One of the goals of Natural Language Understanding (\textit{NLU}) is to address this shortcoming by modelling written text in such a way that linguistic features can be captured.
These models can then be used for other machine learning tasks, consequently referred to as \textit{downstream tasks}.
...
Language representation models, also called Language Models (\textit{LM}) are a category of NLU models that can both generate word embeddings, sentence embeddings, but also intermediate representations that can be used for downstream tasks.
...
Thus, improving the way these embeddings capture linguistic features has strong implications for any subtasks in NLU.
These can include Named Entity Recognition (\textit{NER}), sentiment understanding, and automatic language-translation. 
...
It can also be ambigious, when we use words such as \texttt{bank}.
...
\section{Finding Synsets in Static Word Embeddings}\label{section_clustering}

Most notably \textbf{the Chinese Whispers algorithm} is used to cluster for synsets is described in \cite{pelevina16}.
...
The authors propose a multi-step approach, where they (1) learn static word embeddings, (2) build a graph of nearest neighbours based on vector similarities, (3) apply induction of word senses using ego-network clustering and (4) aggregate the word-vectors with respect to the induced senses.
The authors use the static word2vec word embeddings as the starting step.
A graph is created by only keeping the $N$ nodes with highest weight (denoted by a similarity value).
...
Similarly the JoBimText framework \cite{biemann13}, which provides a graph-based and sparse word similarity model, can be used to cluster for synsets between words. 
...
A more extensive list of datasets separated by knowledge-based approaches and distributional-based approaches can be found in \cite{navigli19}.
...
Due to the different ways semantics can be quantified, different papers show mixed results in extracting a subspace inside of BERT which captures mostly semantic information.
...
They go so far as to say that the position $i$ of the word $w_i$ inside the sequence $s = [w_1, \ldots, w_i, \ldots, w_T]$ has a stronger implication on the position of the produced contextual word embedding than the word token does.
...
Finally, \cite{mickus19} argues that the cosine similarity is a good distance measure for semantic similarity between contextual word embedding vectors, analogous to what was observedin \cite{mikolov13}.
...
In slight contrast to the conclusion of the above work, \cite{wiedmann19} analyses to what extent contextual word embeddings are better at word sense disambiguation tasks.
...
%TODO Replace all PoS with part-of-speech
...
\cite{liebeskind19} create a thesaurus through a co-occurrence-based approach to select a list of first-order candidates, and a list of second-order candidates by using a  distributional-based approach.
However, their approach makes strong use of part-of-speech taggers to disambiguate between word-senses by the presumed correlation to the part-of-speech tag.
...
The authors mention that there are fine-grained geometric representations of word senses, which is encoded in a relatively low-dimensional subspace.
...
Finally, a theorem is presented that shows that a tree with $n$ nodes has a Pythagorean embedding in $\mathbb{R}^{n-1}$.
...
%TODO check tenses in this section ...
...
The biLM architectures learn syntax, as part-of-speech tags are captured at lower levels of the bidirectional LM layers.
...
\cite{caliskan19} follows a similar analysis, focussin much more on the properties of corpora used for training.
...
For words that consist of multiple tokens, \cite{bommasani19, akbik19} do a simple mean, max or min pooling over all token-embeddings to arrive at a word-embedding (both static and contextual word embeddings). \\
